...
const SongList: React.FC<SongListWithPaginationProps> = ({
  ...
  const [sortOrder, setSortOrder] = useState<"relevance" | "asc" | "desc">(
    "relevance"
  );
    ...
    const fetchSongs = async () => {
      try {
        const response = await axios.get(
          `https://us-east-1.aws.data.mongodb-api.com/app/dotstester-bpjzg/endpoint/artistsongs?artistId=${artistId}`
        );
  ...
  const groupSongsByTitle = (songs: Song[]): Song[] => {
    ...
    return Object.values(grouped).map((group) => {
      ...
      return group.sort((a, b) => {
        const aInfo =
          (a["writers"]?.length || 0) + (a["producers-credit"]?.length || 0);
        const bInfo =
          (b["writers"]?.length || 0) + (b["producers-credit"]?.length || 0);
        return bInfo - aInfo;
      })[0];
    });
  };
